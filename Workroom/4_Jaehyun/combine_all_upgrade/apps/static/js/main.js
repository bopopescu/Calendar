...
function drawEventListAtMonthView(first_date_str, last_date_str, event_list, search_query) {
    ...
    for (var i = 0; i < event_list.length; i++) {
        ...
        if( $('.calendar-multiselect > div > ul > li[data-original-index="'+
            cateogryNameEngToOrignialIndex[event_list[i].category_char]+'"].selected').length != 0 ||
            $('.calendar-multiselect > div > ul > li[data-original-index="'+
            event_list[i].category_char+'"].selected').length != 0) {
